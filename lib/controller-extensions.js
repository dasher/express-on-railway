...
Controller.prototype.render = function(arg1, arg2) {
    ...
    var prefixExtendsToTemplate = false;
    if ('prefixExtends' in params || 'prefixExtends' in locals) {
    	prefixExtendsToTemplate = true;
    }
    var file = this.controllerName + '/' + view;
    //var file = path.normalize(this.controllerName + '/' + view).replace(/\\/g, "/"); // Master
    ...
    if (layout) {
    	if (prefixExtendsToTemplate) {
    		var cons = require('consolidate');
    		var jade = require("jade");
    		app.engine("jade", cons.jade);
			var contents = fs.readFileSync(app.root+"/app/views/"+file+".jade", "utf8");
			locals.filename = app.root+"/app/views/"+file+".jade";
			contents = "extends ../"+layout+"\n"+contents;
			
			var compiled = jade.compile(contents, locals);
			compiled=compiled(locals);
			res.send(compiled);
    	} else {
	...
	app.emit("rendered", file, layout, locals, prefixExtendsToTemplate);
};
...
Controller.prototype.load = function(controllerName) {
    ...
    this.compound.app.emit("load controller", controllerName, fullName);
